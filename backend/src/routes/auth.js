...
router.post('/login', async (req, res) => {
    ...
    try {
        ...
        // Fallback: si el seed trae un hash placeholder y el usuario es admin/admin123,
        // se acepta y se reescribe el hash con uno valido.
        if (!ok && username === 'admin' && password === 'admin123') {
            const newHash = await bcrypt.hash('admin123', 10);
            await pool.query('UPDATE usuarios SET password_hash = $1 WHERE id_usuario = $2',
        ...
        res.json({ token, user: { id: user.id_usuario, username: user.username, rol: user.rol } });
    } catch (err) {
        console.error(err);
